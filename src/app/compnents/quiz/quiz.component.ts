...
export class QuizComponent implements OnInit {
  ...
  calculateResult() {
    ...
    if (answers.length < 5) {
      const resultElement = document.getElementById('result');//récupérer l'élément HTML avec l'id "result"
      if (resultElement) {
        resultElement.innerText = "Veuillez répondre à toutes les questions.";//afficher un message si il y a des réponses non répondues
      }
      return;
    }

    //compter le nombre de réponses de chaque type (A, B, C)
    const answerCounts: { [key: string]: number } = { A: 0, B: 0, C: 0 };//tableau pour compter les réponses initialisé à 0
    answers.forEach(answer => {answerCounts[answer]++;}); //incrémenter le nombre de réponses pour la réponse correspondante
